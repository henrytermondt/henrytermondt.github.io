...
class ComputeShaderInput {
    constructor(location, data, width, height, type='RGBA32F') {
        ...
        ComputeShader.gl.texImage2D(ComputeShader.gl.TEXTURE_2D, 0, ComputeShader.gl[type], width, height, 0, ComputeShader.gl.RGBA, ComputeShader.gl.FLOAT, data);
    ...
    update(data) {
        ...
        ComputeShader.gl.texImage2D(ComputeShader.gl.TEXTURE_2D, 0, ComputeShader.gl[this.type], this.width, this.height, 0, ComputeShader.gl.RGBA, ComputeShader.gl.FLOAT, data);
...
class ComputeShaderContext {
    constructor(powerPreference) {
        ...
        this.context = this.canvas.getContext('webgl2', powerPreference ? {powerPreference: 'high-performance'} : {});
    }
    resize(width, height) {
...
class ComputeShader {
    constructor(source, width, height, vertexSource=ComputeShader.vertexShader) {
        if (!ComputeShader.gl) throw new ComputeShaderError(`Attempted to create a compute shader without a WebGL2 context`);
        ...
        this.vertexShader = ComputeShader.createShader(this.gl, this.gl.VERTEX_SHADER, vertexSource);
        ...
        this.output = new ComputeShaderOutput(this.width, this.height, 'RGBA32F');
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.output.texture);
        this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA32F, this.width, this.height, 0, this.gl.RGBA, this.gl.FLOAT, new Float32Array(this.width * this.height * 4));
    ...
    async readAsync(result=new Float32Array(this.width * this.height * 4)) {
        const buf = this.gl.createBuffer();
        this.gl.bindBuffer(this.gl.PIXEL_PACK_BUFFER, buf);
        this.gl.bufferData(this.gl.PIXEL_PACK_BUFFER, result.byteLength, this.gl.STREAM_READ);
        this.gl.readPixels(0, 0, this.width, this.height, this.gl.RGBA, this.gl.FLOAT, 0);
        this.gl.bindBuffer(this.gl.PIXEL_PACK_BUFFER, null);

        await ComputeShader.getBufferSubDataAsync(this.gl.PIXEL_PACK_BUFFER, buf, 0, result);
    ...
    static useContext = context => {
        if (!context.context.getExtension('EXT_color_buffer_float')) throw new ComputeShaderError(`Cannot get extention 'EXT_color_buffer_float'`);
        
        ComputeShader.gl = context.context;
    }
    static gl = null;
    static clientWaitAsync(sync, flags, interval_ms) {
        return new Promise((resolve, reject) => {
            const test = () => {
                const response = ComputeShader.gl.clientWaitSync(sync, flags, 0);
                if (response === ComputeShader.gl.WAIT_FAILED) {
                    reject(new Error('clientWaitSync failed'));
                    return;
                }
                if (response === gl.TIMEOUT_EXPIRED) {
    ...
    static async getBufferSubDataAsync(target, buffer, srcByteOffset, dstBuffer) {
        const sync = ComputeShader.gl.fenceSync(ComputeShader.gl.SYNC_GPU_COMMANDS_COMPLETE, 0);
        ComputeShader.gl.flush();

        await ComputeShader.clientWaitAsync(sync, 0, 10);
        ComputeShader.gl.deleteSync(sync);

        ComputeShader.gl.bindBuffer(target, buffer);
        ComputeShader.gl.getBufferSubData(target, srcByteOffset, dstBuffer);
        ComputeShader.gl.bindBuffer(target, null);
